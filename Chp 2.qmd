...
Trading only happens on weekdays, so it's necessary to re-index with a continuous variable to avoid gaps in the data.
ACF is pure white noise. No pattern discerned in data.
